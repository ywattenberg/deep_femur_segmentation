...
def inference_by_index(slices):
    ...
    model.load_state_dict(torch.load("models/Trab_UNet_epoch_751.pth"))
    # model
    model.eval()
    model.to("cuda")
    ...
    for i in slices:
        ...
        output = output.to('cpu').detach().squeeze().numpy()
