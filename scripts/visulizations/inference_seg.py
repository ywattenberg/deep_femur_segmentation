...
def inference_by_index(slices):
    ...
    model.load_state_dict(torch.load("models/model_weights_retina_UNet.pth"))
    # model
    model.eval()
    model.to("cuda")
    ...
    for i in slices:
        ...
        x = x.unsqueeze(0)
        output_mask, output_image = inferer(x, model)
        output_mask = torch.nn.functional.sigmoid(output_mask).to('cpu').detach().squeeze().numpy()
        output_mask = output_mask > 0.5
        output_image = output_image.to('cpu').detach().squeeze().numpy()
        ...
        ax[0,0].title.set_text("Input")
        ax[1,0].imshow(mask[0, half_depth_x,:,:], cmap="gray")
        ax[1,0].title.set_text("Target 1 ")
        ax[1,1].imshow(output_mask[0,half_depth_x,:,:], cmap="gray")
        ax[1,1].title.set_text("Prediction 1") 
        ax[2,0].imshow(mask[1, half_depth_x,:,:], cmap="gray")
        ax[2,0].title.set_text("Target 2")
        ax[2,1].imshow(output_mask[1, half_depth_x,:,:], cmap="gray")
        ax[2,1].title.set_text("Prediction 2 ")   
        ax[3,0].imshow(y[half_depth_y,:,:], cmap="gray")
        ax[3,0].title.set_text("Target 3")
        ax[3,1].imshow(output_image[half_depth_y,:,:], cmap="gray")
